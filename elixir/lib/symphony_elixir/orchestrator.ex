defmodule SymphonyElixir.Orchestrator do
  ...
  alias SymphonyElixir.{AgentRunner, CodexActivity, Config, StatusDashboard, Tracker, Workspace}
  ...
  defp handle_agent_down(reason, state, issue_id, running_entry, session_id) do
    cond do
      ...
      reason == :normal and input_required_blocker?(running_entry) ->
  ...
  defp block_issue_from_entry(%State{} = state, issue_id, running_entry, error) do
    blocked_entry = %{
      ...
      current_activity: Map.get(running_entry, :current_activity),
      last_meaningful_update: Map.get(running_entry, :last_meaningful_update)
    }
    ...
    running =
      ...
      |> Enum.map(fn {issue_id, metadata} ->
        %{
          ...
          current_activity: Map.get(metadata, :current_activity),
          last_meaningful_update: Map.get(metadata, :last_meaningful_update),
  ...
  defp integrate_codex_update(running_entry, %{event: event, timestamp: timestamp} = update) do
    ...
    updated_running_entry =
      ...
      |> CodexActivity.integrate_running_entry(update)
  ...
  defp schedule_next_terminal_workspace_cleanup(
         %State{} = state,
         now_ms \\ System.monotonic_time(:millisecond)
       )
    ...
    %{
      state
      | next_terminal_workspace_cleanup_due_at_ms: now_ms + @terminal_workspace_cleanup_interval_ms
    }
  end
